...
# Directory for placing rejected files
PHOTOLOGUE_REJECT_DIR = getattr(settings, 'PHOTOLOGUE_REJECT_DIR', '/www/wedding/dropbox')
...
sanitize_filename_allowed = string.ascii_letters + string.digits + "._-: =()"
def sanitize_filename(s, maxlen=48):
    s2 = ""
    while s != "" and s[0] in "./":
        s = s[1:]
    for c in s:
        if c in sanitize_filename_allowed:
            s2 += c
        elif c in "/\\":
            s2 += "__"
        else:
                ...
                if len(data):
                    ...
                    while 1:
                        ...
                        try:
                            ...
                            filepath = os.path.join(gallery.title_slug,sanitize_filename(filename))
                            photo.image.save(filepath, ContentFile(data))
...
class ImageModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        exif = self.EXIF
        if not self.caption:
            try:  self.caption = unicode(exif['Image ImageDescription'])
            except: pass
        if not self.caption:
            try:  self.caption = unicode(exif['EXIF ImageDescription'])
            except: pass
        if not self.caption:
            try:  self.caption = unicode(exif['EXIF UserComment'])
            except: pass
        if not self.caption:
            self.caption = self.image_filename()
...
class Photo(ImageModel):
    ...
    class Meta:
        ordering = ['date_added','date_taken']
        get_latest_by = 'date_added'
