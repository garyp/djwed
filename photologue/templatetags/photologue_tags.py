...
@register.simple_tag
def next_in_gallery_button(photo, gallery):
    next = photo.get_next_in_gallery(gallery)
    if next:
        return '<a alt="NEXT" href="%s">[&gt;&gt;&gt;]</a>' % (next.get_absolute_url())
    return ""
    
@register.simple_tag
def previous_in_gallery_button(photo, gallery):
    prev = photo.get_previous_in_gallery(gallery)
    if prev:
        return '<a alt="PREV" href="%s">[&lt;&lt;&lt;]</a>' % (prev.get_absolute_url(),)
    return ""
